"""
do pre-processing and post-processing
"""
import cv2
import numpy as np
import colorsys
...
def draw_bbox(image, bboxes, classes=None, show_label=True):
    ...
    hsv_tuple = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuple))
...
def postprocess_boxes(pred_bbox, org_img_shape, input_size, score_threshold):
    # pred_bbox are the rearranged result of network outputs
